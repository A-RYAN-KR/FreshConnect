    ...
    const suppliers = await User.aggregate([
      ...
      // Stage 3: (Optional but recommended) Add mock/calculated fields to match the frontend UI
      {
        $addFields: {
          ...
          trustScore: { $floor: { $add: [90, { $multiply: [10, { $rand: {} }] }] } }, // Random trust score 90-99
          rating: { $round: [{ $add: [4.5, { $multiply: [0.5, { $rand: {} }] }] }, 1] }, // Random rating 4.5-5.0
          reviewCount: { $floor: { $add: [50, { $multiply: [150, { $rand: {} }] }] } }, // Random reviews 50-200
          distance: "2.5 km",
